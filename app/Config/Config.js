;(function() {
angular.module('valgrifer.overlay.config', ['ngRoute',
										 'valgrifer.overlay.dpsmeter',
										 'valgrifer.overlay.dbmanager'])

	.factory('presetConfigManager',
		['$localStorage',
			function presetManagerFactory ($storage) {
				let config = {
					get: function getConfig(name) {
						return $storage.settings[name];
					},

					set: function setConfig(name, value) {
					...
					all: function getAll()
					{
						return $storage.settings ? $storage.settings : this.$getDefault;
					},
					...
					$update: function update()
					{
						...
						for (let name in this.all())
							content += "--" + name.replace("_", "-") + ": " + this.get(name) + ";";
						document.querySelector("#rootStyle").innerHTML = ":root {" + content + "}"
					}
				};
			...
			controller:['$scope', '$document', 
				function($scope, $document) {
					...
					$scope.colsCollection = [
						...
						{label:  'Damage(M)',value: 'DAMAGE-m'},{label:  'Damage (%)',value: 'damagePct'},{label:  '% Show Bar',value: 'showbar'},{label:  'dps',value: 'dps'},
		...
		return {
			...
			controller:['$scope', '$parse',
				function fieldselectController($scope, $parse) {
					...
					let parsedOptions = $scope.parsedOptions = []
			...
			controller:['$scope',
				function sorterController ($scope) {
					...
					$scope.up = function up() {
						...
						let move = $scope.ngModel.splice($scope.$index, 1);
